...
class Interface:
    def __init__(self, args, log=None):
        ...
        if not os.path.exists(vocab_file):
            data = []
            for dataset in self.args.data_dir:
                data.extend(load_data(dataset))
            self.target_map = Indexer.build(
                (sample['target'] for sample in data), log=log)
            self.target_map.save(target_map_file)
            self.vocab = Vocab.build(
                (word for sample in data
    ...
    def load_embeddings(self):
        ...
        embedding_file = os.path.join(
            self.args.output_dir, 'embedding.msgpack')
        if not os.path.exists(embedding_file):
            embeddings = load_embeddings(
                self.args.pretrained_embeddings, self.vocab,
    ...
    def pre_process(self, data, i, training=True, with_target=True):
        result = [
            self.process_sample(sample, i, with_target) for sample in data
        ]
        if training:
            result = list(
                filter(
                    lambda x: len(x['text1']) < self.args.max_len and len(
                        x['text2']) < self.args.max_len, result
                )
            )
            if not self.args.sort_by_len:
                return result
            result = sorted(result, key=lambda x: (
                len(x['text1']), len(x['text2']), x['text1']))
        batch_size = self.args.batch_size[i]
        return [self.make_batch(result[i:i + batch_size]) for i in range(0, len(data), batch_size)]

    def process_sample(self, sample, i, with_target=True):
        text1 = sample['text1']
        text2 = sample['text2']

        sen1 = tokenizer.encode(text1)[:self.args.max_len1[i]]
        sen2 = tokenizer.encode(text2)[:self.args.max_len2[i]]

        sen1.insert(0, 101)
        sen1.insert(self.args.max_len1[i]+1, 102)
        sen2.insert(self.args.max_len2[i]+1, 102)
    ...
    def make_batch(self, batch, with_target=True):
        ...
        if 'target' in batch and not with_target:
            del batch['target']
        batch = {
            key: self.padding(value, min_len=self.args.min_len) if key.startswith(
                'text') else value
    ...
    def padding(samples, min_len=1):
        ...
        batch = [
            sample + [Vocab.pad()] * (max_len - len(sample))
