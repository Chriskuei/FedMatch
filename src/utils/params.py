...
def _post_process(args: Object):
    ...
    def samples2steps(n, i):
        return int(math.ceil(n / args.batch_size[i]))

    totals = len(args.data_dir)
    if not hasattr(args, 'log_per_updates'):
        args.log_per_updates = []
        for i in range(totals):
            args.log_per_updates.append(samples2steps(args.log_per_samples[i], i))
    if not hasattr(args, 'eval_per_updates'):
        args.eval_per_updates = []
        for i in range(totals):
            args.eval_per_updates.append(samples2steps(args.eval_per_samples[i], i))
    if not hasattr(args, 'eval_per_updates_warmup'):
        args.eval_per_updates_warmup = []
        for i in range(totals):
            args.eval_per_updates_warmup.append(samples2steps(args.eval_per_samples_warmup[i], i))
    if not hasattr(args, 'eval_warmup_steps'):
        args.eval_warmup_steps = []
        for i in range(totals):
            args.eval_warmup_steps.append(samples2steps(args.eval_warmup_samples[i], i))
    if not hasattr(args, 'min_steps'):
        args.min_steps = []
        for i in range(totals):
            args.min_steps.append(samples2steps(args.min_samples[i], i))
    if not hasattr(args, 'early_stopping'):
        args.early_stopping = []
        for i in range(totals):
            args.early_stopping.append(samples2steps(args.tolerance_samples[i], i))
    if not hasattr(args, 'summary_per_updates'):
        args.summary_per_updates = [args.summary_per_logs * log_per_updates for log_per_updates in args.log_per_updates]
