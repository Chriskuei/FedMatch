...
class BertConfig(object):
    ...
    def __init__(self,
        ...
        self.top_pals = top_pals
        self.top_low_rank = top_low_rank
        self.vertical = vertical
        self.vertical_pals = vertical_pals
        self.vertical_low_rank = vertical_low_rank
...
class BERTEncoder(nn.Module):
    def __init__(self, config):
        ...
        if config.top:
            ...
            elif config.top_pals:
                multi = BERTPals(config)
            elif config.top_low_rank:
                multi = BERTLowRank(config)
            else:
    ...
    def forward(self, hidden_states, attention_mask, i=0):
        ...
        if self.config.top:
            ...
            elif self.config.top_pals or self.config.top_low_rank:
                all_encoder_layers[-1] = self.multi_layers[i](hidden_states, attention_mask)
            else:
...
class BertModel(nn.Module):
    ...
    def __init__(self, config: BertConfig):
        ...
        if config.vertical:
            if config.vertical_pals:
                self.vertical = BERTPals(config)
            elif config.vertical_low_rank:
                self.vertical = BERTLowRank(config)
            else:
                vertical_config = copy.deepcopy(config)
                vertical_config.num_hidden_layers = vertical_config.vertical_num_hidden_layers
                self.vertical = BERTEncoder(vertical_config)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, i=0):
        ...
        if self.config.vertical:
            if self.config.vertical_pals or self.config.vertical_low_rank:
                vertical_output = self.vertical(embedding_output, extended_attention_mask)
            else:
                all_vertical_layers = self.vertical(embedding_output, extended_attention_mask, i)
                vertical_output = all_vertical_layers[-1]
            # sequence_output = sequence_output + vertical_output
            sequence_output = self.LayerNorm(sequence_output + vertical_output)
